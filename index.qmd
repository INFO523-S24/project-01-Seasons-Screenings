...
df_earnings['earnings'] = df_earnings['earnings'] // 1000000  # earnings in mln $
df_earnings.head()
```

We can observe that there are more outliers before 1980. There are both extremely high earners and movies that made very little. There is less data on older movies, so this information has to be taken with a grain of salt. It could be simply a result of too few datapoints.
```{python}
...
As we may intuitively expect, there indeed seems to be a correlation between the average rating of a movie and its earnings. The more people like the movie, the more tickets it sells.
```{python}
