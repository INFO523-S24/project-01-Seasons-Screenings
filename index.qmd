...
plt.figure(figsize=(12, 8))
sns.lineplot(x="title_length", y="average_rating", data = top_movies, color = "purple")
...
merged_data['genres_list'] = merged_data['genresy'].str.split(',')
...
merged_data = pd.merge(df[['tconst', 'runtime_minutes']], df_earnings[['tconst', 'earnings']], on='tconst', how='inner')

# Now that we have a merged DataFrame with runtime and earnings, we can group by runtime and calculate average earnings
duration_earnings = merged_data.groupby('runtime_minutes')['earnings'].mean().reset_index()
...
holiday_earnings['Average Earnings (in million USD)'] = holiday_earnings['Average Earnings'] / 1e6

# Create a bar plot to show average earnings by holiday type
plt.figure(figsize=(12, 8))
sns.barplot(x='Average Earnings (in million USD)', y='Holiday Type', data=holiday_earnings)
plt.xlabel('Average Earnings (in million USD)')
...
The bar chart illustrates that Christmas-themed movies are the highest earners, followed by films about holidays in general. Movies about Hanukkah and Kwanzaa have notably lower average earnings, suggesting that Christmas films are the most commercially successful within holiday genres.